...
The fifth is the app itself, and it needed a browser. The deletion pass (Förr,
search, the layer menu, POI overlays, mode tabs) landed from a sandbox that
could not boot it, so `test/smoke.test.mjs` boots it headless with Playwright
and checks the four things nobody had looked at: the quiz front door with no
mode tabs, the explore map with no search bar or layer button or POI dots,
tapping things opens the card and tapping water closes it, and starting a round
blinds the map with no text leaking through. It skips when `playwright-core` is
not installed or the build artifacts are missing.

`test/areas.test.mjs`, `test/style.test.mjs`, `test/rounds.test.mjs`,
`test/streets.test.mjs`, `test/blind.test.mjs` and `test/smoke.test.mjs` read
`build/` and `data/`,
