...
- **The map carries no pins, and the layer menu is gone** (2026-08-23). The map
  is a board to be tested on, not a thing to look something up in. A café dot is
  an answer to a question this app does not ask — "where is the nearest lunch"
  is navigation, and navigation is the one thing this map is explicitly not. So
  every OSM POI layer stays off the basemap, and the curated landmarks are the
  only points drawn. They are always on, because they are the fixed points the
  quiz asks you to find, not a layer to be negotiated with.

  The layer menu went with them, and for the same reason: its fourteen chips
  were a reference-map feature, and the reference map is no longer the app — it
  is the escape hatch you study in, and the study aid is the labels already on
  it. A menu of toggles for things that are either always on (landmarks,
  kvarter names, roads) or not on the map at all (POIs) is a menu that offers
  nothing, and a bottom-of-the-phone menu that offers nothing is a menu you
  read past every time you look at the city. The kvarter names lost their chip
  and became always-on too: they are names the map is for, and a chip to hide
  them was a chip to hide the answer to "what is this place called".
...
- **Fully offline PWA** (2026-07-17). Offline looked ruled out by tileset size.
  Reality: a 13.3 MB basemap, ~21.5 MB total payload — small enough to load the
...
- **The style is carved from OSM Liberty, by script, from a pinned commit**
  ...
  drops 20 layers, retunes 20 and adds 1 — each with a one-line reason in the
